...
public class bookForm extends javax.swing.JInternalFrame {
    ...
    public bookForm() {
        ...
        initComponents();
        viewJTableBook();
        viewJTableBookCategory();
        
        
    }
    
    public void viewJTableBook(){
    ...
    public void viewJTableBookCategory(){
        model = (DefaultTableModel) bookCategoryTable.getModel();
        model.setRowCount(0);
        List<BookCategory> all = new BookCategoryDao().findAll();
    ...
    private void saveButtonBookActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonBookActionPerformed
        ...
        java.util.Date dateOfPublication = dateOfPublicationField.getDate();
        //Date dateOfPublication = (Date) dateOfPublicationField.getDate();
        String author = authorField.getText();
        String pages = pageField.getText();
        int page = Integer.parseInt(pages);
        String bookCategory = bookCategoryField.getText();

        Book bk = new Book(bookId, title, publishingHouse, author, author, pages, bookCategory);
        
        BookCategoryDao bcd = new BookCategoryDao();
        
        List<BookCategory> all = bcd.findAll();
        
        for (BookCategory bookCategory1 : all) {
            if(bookCategory1.getBookCategoryName().equalsIgnoreCase(bookCategory)){
                bk.setBookCategory(bookCategory1);
            }
        }
